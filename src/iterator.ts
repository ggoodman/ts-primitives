...
export const Iterator = {
  ...
  fromArray<T>(array: T[], index = 0, length = array.length): Iterator<T> {
    return {
      next(): IteratorResult<T> {
        if (index >= length) {
          return FIN;
  ...
  from<T>(elements: Iterator<T> | T[] | undefined): Iterator<T> {
    if (!elements) {
  ...
  map<T, R>(iterator: Iterator<T>, fn: (t: T) => R): Iterator<R> {
    return {
      next() {
  ...
  filter<T>(iterator: Iterator<T>, fn: (t: T) => boolean): Iterator<T> {
    return {
      next() {
        while (true) {
          const element = iterator.next();
  ...
  forEach<T>(iterator: Iterator<T>, fn: (t: T) => void): void {
  ...
  collect<T>(iterator: Iterator<T>): T[] {
    const result: T[] = [];
    Iterator.forEach(iterator, value => result.push(value));
    return result;
  },
};
